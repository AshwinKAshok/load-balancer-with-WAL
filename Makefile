...
SUBDIRS=load_balancer

load-balancer::
	        @echo make load-balancer
...
$(SVR_OBJS): $(SVR_SRCS) $(SVR_HDRS)
	$(CXX) $(CFLAGS) $(DFLAGS) -c $(SVR_SRCS) $(LDLIBS)
...
clean:
	rm -rf *.o $(TARGET) ; @echo make clean $(foreach var,$(SUBDIRS),echo $(var): ; cd $(var)/ && make $@ && cd ..;)
